 ...
 */
// إزالة dotenv/config لأن المتغيرات تأتي من Cloud Run مباشرة
// وإضافتها قد تسبب انهياراً إذا لم تكن في Dependencies
...
// تحسين جلب متغيرات البيئة مع قيم افتراضية أو رسائل تحذير
const GOOGLE_CLOUD_LOCATION = process.env.GOOGLE_CLOUD_LOCATION || 'me-west1';
const GOOGLE_CLOUD_PROJECT = process.env.GOOGLE_CLOUD_PROJECT;
const PROXY_HEADER = process.env.PROXY_HEADER;

console.log(`[Startup] Project: ${GOOGLE_CLOUD_PROJECT}, Location: ${GOOGLE_CLOUD_LOCATION}`);

if (!GOOGLE_CLOUD_PROJECT || !PROXY_HEADER) {
  console.warn("WARNING: Essential environment variables (GOOGLE_CLOUD_PROJECT or PROXY_HEADER) are missing.");
  console.warn("The server will start but AI features might fail.");
}
...
// نقطة فحص الصحة (Health Check) لنظام Google Cloud
app.get('/health', (req, res) => res.status(200).send('OK'));

// تفعيل عرض ملفات الواجهة الأمامية (React)
const publicPath = path.join(__dirname, 'public');
app.use(express.static(publicPath));

// التأكد من توجيه أي مسار غير معروف إلى تطبيق React (لدعم SPA Routing)
app.get('*', (req, res, next) => {
  ...
  res.sendFile(indexPath, (err) => {
    if (err) res.status(200).send('Server is running, but frontend files are missing.');
  });
});
...
// معالجة أخطاء بدء التشغيل (مثل تعارض المنفذ) بشكل صحيح
server.on('error', (err) => {
...
server.on('upgrade', async (request, socket, head) => {
  // تأمينconstruct URL لمنع الانهيار إذا كان الـ Header مفقوداً
  let url;
  try {
